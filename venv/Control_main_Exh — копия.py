...
def hendlerDB2(): # функция, которая выполняет обработку базы данных, csv файла(UserLog), txt файла(ConnectionDB)
    # считывание и запись в БД параметров эксгаустера №3
    predict_Exh3()
...
def predict_Exh3():
    ...
    time_min_alarm = min(Exh3_Alarm_Vibr1.predictor, Exh3_Alarm_Vibr2.predictor, Exh3_Alarm_Temp1.predictor,
                         Exh3_Alarm_Temp2.predictor)
    ...
    if txt_time_ponit_norm < date_time_now_norm:  # делаю именно такое сравнение, чтобы не перегружать БД и записывать только изменения
        ...
        data_alarm = [
            [date_time_now, Exh3_Alarm_Vibr1.predictor, Exh3_Alarm_Vibr2.predictor, Exh3_Alarm_Temp1.predictor,
             Exh3_Alarm_Temp2.predictor, time_min_alarm, name_alarm, time_count_alarm]]
        # print(data_alarm)
        ############################################# запишу данные в БД ###################################################
...
def exh3_Alarm_Vibr1():
    ...
    try:
        ...
        temp_str2 = 'select TagTime, TagValue  from Exh3_Temp1 where TagTime <= ' + "'" + temp_x2_1 + "'" + ' and ' + 'TagTime >= ' + "'" + temp_x2_2 + "'"
        ...
        temp_str1 = 'select TagTime, TagValue  from Exh3_Temp1 where TagTime <= ' + "'" + temp_x1_1 + "'" + ' and ' + 'TagTime >= ' + "'" + temp_x1_2 + "'"
        ...
        # *******************************************************************************************
        mean = 85  # уставка, при достижении, которой необходимо узнать дату и время *************
        # *******************************************************************************************
        if mean1 > mean2:
            ...
            exh3_Alarm_Vibr1_predictor = ((mean - mean1) * (x2_1 - x1_1)) / (mean2 - mean1) + x1_1
            # print('predictor', predictor)

            # print('дата и время выхода из строя ротора', (str(predictor))[:19])

            time_to_alarm = exh3_Alarm_Vibr1_predictor - x2_1
            str_time_to_alarm = (str(time_to_alarm))[:16]

            print('Количество дней до выхода из строя ротора по вибрации т.1', str_time_to_alarm)
        #print(77777888888)
        #conn.commit()  # сохраняем изменения в БД
        #print(44444455555)
        #conn.close()  # закрываем подключение к базе
...
print("Время цикла выполнения программы всей программы:", (end - start) * 10 ** 3, "ms")
