...
with open('conn_file_Exh3_Temp3.txt', 'r', encoding='utf-8') as file:
...
csv_list = []
with open('Exh3_Temp3.csv', 'r', newline='', encoding='ANSI') as f:
...
if len(data_kafka) > 1:
    ...
    try:
        ...
        cur.execute("select TagTime, TagValue  from Exh3_Temp3 where TagTime = (select MAX(TagTime) from Exh3_Temp3)")  # записываю в курсор максимальное значение времени из БД
        #print(1)
        ...
        if (db_list[0][0] == data_kafka[0][0]):
            ...
            with open('conn_file_Exh3_Temp3.txt', 'r', encoding='utf-8') as fr:
                lines = fr.readlines()
            ...
            #print(lines)
            with open('conn_file_Exh3_Temp3.txt', 'w', encoding='utf-8') as fl:
                #print(88)
                fl.writelines(lines)
