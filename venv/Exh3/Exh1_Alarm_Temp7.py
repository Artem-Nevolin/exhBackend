...
with open('conn_file/conn_file_Exh1_Temp7.txt', 'r', encoding='utf-8') as file:
...
try:
    ...
    cur.execute("select TagTime, Nag1_temp7  from Exh_All")  # записываю в курсор максимальное значение времени из БД
...
# ********************************** делаю выгрузку данных за текущий час для нахождения среднего значения **********
    temp_str2 = 'select TagTime, Nag1_temp7  from Exh_All where TagTime <= ' + "'" + temp_x2_1 + "'" + ' and ' + 'TagTime >= ' + "'" + temp_x2_2 + "'"
...
# ********************************** делаю выгрузку данных за 12 час для нахождения среднего значения **********
    ...
    temp_str1 = 'select TagTime, Nag1_temp7  from Exh_All where TagTime <= ' + "'" + temp_x1_1 + "'" + ' and ' + 'TagTime >= ' + "'" + temp_x1_2 + "'"
...
################################ нахожу среднее значение в первый и последний(12) час ##############################
    ...
    if len(mean_point_1)>0:
        ...
        # *******************************************************************************************
        mean = 75  # уставка, при достижении, которой необходимо узнать дату и время *************
        # *******************************************************************************************
        ...
        predictor = '9999-09-09 09:09:09'
        print('predictor не достаточно данных', predictor)
    conn.commit()  # сохраняем изменения в БД
